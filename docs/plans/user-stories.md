...
**As the** dlp-agent
**I want** to spawn one Tauri DLP UI subprocess in each active user session when the service starts, and in any new session that connects thereafter
...
- [ ] For each active session, Agent calls `CreateProcessAsUser` with that session's token to launch one Tauri UI on that session's desktop
