...
| T-11 | Implement `InterceptionEngine` trait + `file_monitor.rs`: `notify` crate (`ReadDirectoryChangesW`) watching configured paths for file create/write/delete/move events                            | `dlp-agent/src/interception/file_monitor.rs` |
...
| T-37 | Implement process protection DACL: `SetKernelObjectSecurity` on Agent and UI process handles; deny `PROCESS_TERMINATE`, `PROCESS_CREATE_THREAD`, `PROCESS_VM_OPERATION`, `PROCESS_VM_READ`, `PROCESS_VM_WRITE` to `Everyone` SID; SYSTEM retains full access through inherited ACEs | `dlp-agent/src/protection.rs`                      |
| T-38 | Implement password-protected service stop: `sc stop` → STOP_PENDING → send PASSWORD_DIALOG over Pipe 1 → collect PASSWORD_SUBMIT → DPAPI `CryptUnprotectData` → bcrypt verify against `DLPAuthHash` registry value → clean shutdown; 3 wrong attempts → log EVENT_DLP_ADMIN_STOP_FAILED | `dlp-agent/src/service.rs`                         |
...
- [ ] On startup, Agent applies a DACL to its own process denying `PROCESS_TERMINATE`, `PROCESS_CREATE_THREAD`, `PROCESS_VM_OPERATION`, `PROCESS_VM_READ`, `PROCESS_VM_WRITE` to `Everyone` SID
- [ ] Agent applies the same DACL to the UI subprocess after spawning it
- [ ] A DENY ACE for `Everyone` SID blocks `PROCESS_TERMINATE`, `PROCESS_CREATE_THREAD`, `PROCESS_VM_OPERATION`, `PROCESS_VM_READ`, `PROCESS_VM_WRITE`; SYSTEM retains full access through inherited ACEs
...
- [ ] On Submit: UI sends PASSWORD_SUBMIT over Pipe 1; Agent validates credentials via bcrypt hash comparison against `DLPAuthHash` registry value
...
| ID   | Story        | Task                                                                                                                                                                             | Deliverable                                  |
| ---- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| T-11 | US-07        | Implement `InterceptionEngine` trait + `file_monitor.rs`: `notify` crate (`ReadDirectoryChangesW`) watching configured paths for file create/write/delete/move events                      | `dlp-agent/src/interception/file_monitor.rs` |
...
| T-37 | US-A5        | Implement process protection DACL: `SetKernelObjectSecurity` on Agent and UI process handles; deny `PROCESS_TERMINATE`, `PROCESS_CREATE_THREAD`, `PROCESS_VM_OPERATION`, `PROCESS_VM_READ`, `PROCESS_VM_WRITE` to `Everyone` SID; SYSTEM retains full access through inherited ACEs | `dlp-agent/src/protection.rs`                      |
| T-38 | US-A6        | Implement password-protected service stop: `sc stop` → STOP_PENDING → send PASSWORD_DIALOG over Pipe 1 → collect PASSWORD_SUBMIT → DPAPI `CryptUnprotectData` → bcrypt verify against `DLPAuthHash` registry value → clean shutdown; 3 wrong attempts → log EVENT_DLP_ADMIN_STOP_FAILED | `dlp-agent/src/service.rs`                         |
...
| Sprint 8  | US-A5               | T-37                         | Process DACL protection: DENY ACE for `Everyone` SID on Agent and UI processes                                       |
| Sprint 9  | US-A6               | T-38                         | Password-protected service stop: DPAPI + bcrypt hash verify + STOP_PENDING flow                                   |
...
| Sprint 14 | US-09, US-10        | T-13, T-14, T-15             | USB detection + SMB share detection via MPR polling (`WNetOpenEnumW`)                                              |
