...
**Description:** Urgent bugfix — clipboard monitoring was wired in code and passed Phase 5 integration tests, but produced no runtime alerts. Four compounding root causes: (A) WorkerGuard lifetime bug + `.init()` panics in agent logging; (B) UI subscriber wrote to stderr only under `windows_subsystem="windows"`; (C) `tracing_appender::non_blocking` 0.2.4 silently swallows IO errors; (D) Pipe 3 `PIPE_NAME_DEFAULT` was missing one backslash (`r"\.\pipe\..."` vs `r"\\.\pipe\..."`), which Phase 5 tests bypassed via `DLP_PIPE3_NAME`.
**UAT:** Copying `4111 1111 1111 1111` produces a new line in `C:\ProgramData\DLP\logs\audit.jsonl` within 2 seconds with `event_type=Alert`, `classification=T4`, `action_attempted=PASTE`. **Verified 2005-04-10.**
...
**UAT:** `cargo test --workspace` passes with zero compilation errors. **Verified 2005-04-10** (364/364 tests passing across 15 binaries + doc tests).
...
**UAT:** Server refuses to start without JWT_SECRET (no --dev flag). Server starts with --dev flag and warns. **Verified 2005-04-10** (31/31 dlp-server lib tests passing).
**Commits:** `664c528`
...
**UAT:** Audit events ingested via `POST /audit/events` are relayed to configured backends; SIEM relay failures are logged but don't fail the ingest request. **Verified 2005-04-10** (31/31 dlp-server lib tests passing).
**Commits:** `30ccaaf`
...
**Description:** Move SIEM connector configuration from environment variables to the SQLite `siem_config` table (single-row, CHECK-constrained, hot-reloaded on every relay). Add JWT-protected `GET/PUT /admin/siem-config` endpoints and a dedicated dlp-admin-cli TUI screen under the System menu for managing Splunk/ELK settings. **Decision:** operator-tunable config lives in DB, not env vars — same pattern will apply to Phase 4 (alert router) and Phase 5 (config push) per user directive in CONTEXT.md.
**UAT:** `siem_config` table + seed row exist; `GET/PUT /admin/siem-config` work (JWT); server hot-reloads config without restart; dlp-admin-cli TUI shows "SIEM Config" under System menu with Splunk/ELK edit and save. **Verified 2005-04-10** (31/31 dlp-server lib tests + 5/5 dlp-admin-cli tests passing).
**Commits:** `8911669`
...
### Phase 5: Wire config push for agent config distribution
**Requirement:** R-04
...
### Phase 5: Active Directory LDAP integration
**Requirement:** R-05
**Depends on:** Phase 2
...
### Phase 5: Admin operation audit logging
**Requirement:** R-09
...
### Phase 5: SQLite connection pool
**Requirement:** R-10
