...
re_verification:
  ...
  regressions: []
gaps:
  - truth: "Unit tests for new disable_usb_device and enable_usb_device behavior exist"
    status: failed
    reason: "Plan 02 acceptance criteria required test_map_resolution_error, test_disable_usb_device_signature_compiles, and test_enable_usb_device_signature_compiles. None exist in device_controller.rs. Only 5 pre-existing DACL cache tests are present."
    artifacts:
      - path: "dlp-agent/src/device_controller.rs"
        issue: "Missing 3 unit tests for new signature and error mapping"
    missing:
  ...
  - truth: "Unit tests for apply_tier_enforcement D-06/D-07 behavior exist"
    status: failed
    reason: "Plan 03 acceptance criteria required tests for apply_tier_enforcement signature, pnp_result/dacl_result spans, and unified error. No such tests exist in detection/usb.rs."
    artifacts:
      - path: "dlp-agent/src/detection/usb.rs"
        issue: "Missing unit tests for new apply_tier_enforcement Result return, D-06 both-failure path, and D-07 structured tracing spans"
    missing:
      - "Test for apply_tier_enforcement signature compiles with dbcc_name parameter"
      - "Test for D-06 unified error when both PnP and DACL fail"
      - "Test for D-07 structured span fields (pnp_result, dacl_result)"
  - truth: "Unit tests for find_instance_id_by_vid_pid_serial (none) serial handling exist"
    status: failed
    reason: "Plan 01 acceptance criteria required tests for (none) serial fallback (D-04) and ambiguous match rejection (D-05). Only a signature-compiles test exists."
    artifacts:
      - path: "dlp-common/src/usb.rs"
        issue: "Missing tests for D-04 (none) serial fallback and D-05 ambiguous match rejection"
    missing:
      - "test_find_instance_id_none_serial_fallback"
      - "test_find_instance_id_ambiguous_none_serial_returns_err"
---
...
| 9 | All tests pass | VERIFIED | `cargo test -p dlp-common --lib`: 120 passed. `cargo test -p dlp-agent --lib`: 267 passed. `cargo clippy -p dlp-agent -- -D warnings`: No issues. `cargo clippy -p dlp-common -- -D warnings`: No issues. `cargo build -p dlp-agent` and `cargo build -p dlp-common`: 0 warnings. |
| 10 | No compiler warnings | VERIFIED | Both crates compile with zero warnings. Clippy passes with `-D warnings`. |
| 11 | Unit tests cover new functionality (disable/enable signatures, error mapping, D-06/D-07 paths) | FAILED | See Gaps section below. |

**Score:** 8/11 truths verified (3 failed due to missing tests)
...
|----------|---------|--------|--------|
| dlp-common tests pass | `cargo test -p dlp-common --lib` | 120 passed | PASS |
| dlp-agent tests pass | `cargo test -p dlp-agent --lib` | 267 passed | PASS |
...
Three gaps were found, all related to **missing unit tests** for new functionality introduced in this phase:

1. **device_controller.rs missing tests (Plan 02):** The PLAN.md acceptance criteria explicitly required `test_map_resolution_error_config_manager`, `test_disable_usb_device_signature_compiles`, and `test_enable_usb_device_signature_compiles`. None of these tests exist. The 5 existing tests in device_controller.rs only cover DACL cache behavior (pre-existing functionality).

2. **detection/usb.rs missing tests (Plan 03):** The PLAN.md acceptance criteria required tests for `apply_tier_enforcement` signature with `dbcc_name`, D-06 unified error path, and D-07 structured tracing spans. No such tests exist. The 17 existing tests cover reconciliation logic and drive letter handling but not the new enforcement pipeline behavior.

3. **dlp-common usb.rs missing tests (Plan 01):** The PLAN.md acceptance criteria required tests for `(none)` serial fallback (D-04) and ambiguous match rejection (D-05). Only a signature-compiles test and a malformed-path rejection test exist. The critical `(none)` serial handling paths are untested.

**Root cause:** The SUMMARY.md files for all 3 plans claim "All tests pass" but do not acknowledge that the newly-required tests were never added. The test counts (120 for dlp-common, 267 for dlp-agent) match pre-phase counts plus existing tests -- no new tests were actually written.

**Impact:** The core functionality is correctly implemented and compiles. The missing tests are a quality gap that should be closed before proceeding, per the project's testing standards (CLAUDE.md section 9.8: "MUST write unit tests for all new functions and types").
