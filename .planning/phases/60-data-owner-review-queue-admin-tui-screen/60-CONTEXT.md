...
### Scope Boundary
- **Q1:** Phase 60 adds audit events + Data Owner filtering + notification hooks. The UI (LabelReviewQueue screen) already exists from Phase 59-04.
- **Q2:** Reuse `GET /admin/labels?state=temporary` — do NOT create a separate `/admin/labels/pending` alias. Add `department` filter to the existing `LabelFilter` query struct.
- **Q3:** Add `scanner_confidence` column to `labels` table now (f32, nullable, default NULL) so the v0.12.0 scanner can populate it without a schema migration.
- **Q4:** Confirm/reject emit standard SIEM audit events via the existing `audit_log` table with `action = "label_confirmed"` or `"label_rejected"`.

### Data Owner Access Model
- **Q1:** Data Owners access the queue through the same `dlp-admin-cli` TUI. No separate tool.
- **Q2:** Non-admin Data Owners can open the TUI if they belong to the AD `dlp-data-owners` group. Their queue view is read-only (confirm/reject only, no create/edit/delete).
- **Q3:** Admins see all labels in the queue; Data Owners see only labels where `owner_sid` matches their authenticated AD SID.

### Workflow Integration
- **Q1:** Confirm triggers label resolution cache invalidation (`LabelCache` RwLock clear) so ABAC enforcement sees the new tier within the next evaluation.
- **Q2:** Rejected labels remain in `rejected` state (audit trail preserved). Optional hard delete is admin-only via existing `DELETE /admin/labels/:id`.
- **Q3:** No auto-expiry in Phase 60. Expiry logic deferred to Phase 61 (Approval Workflow Engine).

### Claude's Discretion
- Department extraction from AD SID: use the SID's domain prefix as a lightweight department proxy, or require explicit `department` field on labels. Prefer explicit field for correctness.
- Notification hooks: emit a placeholder event (e.g., `alert_router::send` with `alert_type = "label_pending_review"`) that Phase 62 (Syslog Forwarder) or Phase 68 (Email) can consume. Do not build a full notification pipeline.
