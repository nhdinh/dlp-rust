...
## Phase 61 Plan Review — Cycle 3 (Revised Plans)

### Summary

The revised plans represent a substantial improvement over the initial drafts. All five original HIGH-severity concerns (missing creation endpoints, Ed25519 API compatibility, cache key mismatch, token delivery gap, and unencrypted signing key) are genuinely resolved with concrete tasks and verifiable acceptance criteria. The plans now include a compilation spike, encrypted key storage, path-to-UUID resolution, server push + agent startup sync, and comprehensive TOCTOU guards. However, **three new blockers** have emerged or remain from the Cycle 2/3 reviews: a **cross-crate type dependency** that will fail compilation (`CachedApproval` in `dlp-common` referencing `ApprovalClaims` in `dlp-server`), an **incomplete TOCTOU guard** for revocation (`update_state` hardcodes `WHERE status='pending'` but is used for revoke which needs `'approved'`), and **no offline JWT verification mechanism** for the agent (no public key distribution endpoint). These must be fixed before execution.

### Strengths

- **Ed25519 compilation spike (Task 0)** correctly de-risks the entire token service before any implementation code is written
- **TOCTOU guard with `WHERE status='pending'` + `rows_affected` check** prevents double-grant races
- **Encrypted key storage via Phase 47 Envelope** eliminates raw-env-var exposure
- **Cache key includes `destination_scope`** with `scope_matches` wildcard validation prevents USB-drive scope bypass
- **JWT re-verification on every cache read** eliminates cache-poisoning as an attack vector
- **Agent startup sync (`GET /agent/approvals/active`)** prevents token loss after restart
- **T4 canonical message with `jti` anti-replay** is cryptographically sound
- **Explicit three-stage ABAC pipeline** documented in `evaluate()`: NTFS -> ABAC -> approval override
- **Wave ordering (01 -> 02/03 -> 04)** correctly isolates foundation from integration from UI

### Concerns

**HIGH — Cross-crate type dependency will fail compilation**
`dlp-common/src/approval.rs` defines `CachedApproval { claims: ApprovalClaims, ... }` (Plan 01 Task 1), but `ApprovalClaims` is defined in `dlp-server/src/approval_token.rs` (Plan 01 Task 3). Since `dlp-server` depends on `dlp-common`, not vice versa, this creates an unresolvable circular dependency. The plan must either move `ApprovalClaims` to `dlp-common` or change `CachedApproval` to store claims fields directly.

**HIGH — Agent cannot verify approval tokens offline**
There is no mechanism for the agent to obtain the server's Ed25519 public key. `check_approval` re-verifies JWT signatures on every cache read (Plan 03 Task 1), but the agent has no `ApprovalTokenService` with the verifying key. If the agent is offline, cached approvals cannot be validated. A `GET /agent/approvals/public-key` endpoint and agent-side key caching is needed.

**MEDIUM — Revoke TOCTOU guard is functionally broken**
Plan 01 Task 2 defines `update_state` with `WHERE status = 'pending'` hardcoded in the SQL. Plan 02 Task 2 uses this same method for `revoke_approval`, which must match `status = 'approved'`. The current plan would cause revoke to silently no-op (returning 0 rows) because the WHERE clause mismatches. The method needs an `expected_current_status` parameter, or separate transition methods.

**MEDIUM — `/agent/approval-token` endpoint location contradicts push architecture**
Plan 02 says the server "pushes token to agent via HTTP POST to `/agent/approval-token`", but Plan 03 Task 2 defines this handler in `dlp-server/src/agent_api.rs` — a server file. If the server pushes to the agent, the handler belongs in the agent's HTTP server, not the server's API. If the agent polls the server, there is no push mechanism. The plans contradict each other on where this endpoint lives and who calls it.

**MEDIUM — Approval audit event types not wired into SIEM routing**
Plan 02 Task 1 adds `ApprovalEventType` variants but does not explicitly specify integration with `routed_to_siem()` or `triggers_alert()`. Without explicit wiring instructions, the implementer may miss connecting approval events to the existing audit pipeline.

**MEDIUM — Migration mechanism may conflict with existing conventions**
Plan 01 Task 4 introduces `PRAGMA user_version` for schema versioning. OpenCode Cycle 3 noted the existing codebase may already use `run_alter()` inside `run_migrations()` for additive changes. Introducing a parallel migration mechanism risks drift. The plan should verify and align with the existing convention before execution.

**LOW — `PolicyStore` type referenced from agent code**
Plan 03 Task 2's `sync_active_approvals` takes `policy_store: &PolicyStore`, but `PolicyStore` is defined in `dlp-server`. The agent crate cannot reference server types. The agent needs its own local approval cache type (e.g., `ApprovalCache` in `dlp-agent`).

**LOW — T4 board signature out-of-band channel unspecified**
The canonical message is displayed in the TUI detail screen (Plan 04 Task 2), but there is no mechanism for a board member (who is typically not the admin operating the TUI) to receive this message, sign it with their private key, and return the hex signature. An operational note about the OOB channel is needed.

**LOW — `chrono::Duration::hours` may not exist**
Plan 04 Task 3 uses `chrono::Duration::hours(form.expiry_hours as i64)`. Depending on the chrono version, this may need to be `chrono::TimeDelta::try_hours()` or `chrono::Duration::try_hours()`.
...
8. **Add an operational note** about how board members receive canonical messages (email, secure chat, etc.) and return signatures

### Risk Assessment

**MEDIUM**

The core cryptographic architecture, TOCTOU guards, cache strategy, and wave ordering are sound. The Ed25519 spike, encrypted key storage, and startup sync all represent genuine risk reduction. However, the **cross-crate type dependency** is a hard compilation blocker that would stall Plan 01 mid-execution, the **revoke TOCTOU bug** would produce a subtle functional defect (revoke silently failing), and the **offline verification gap** means the approval system fails during network partitions — a realistic enterprise scenario. These three issues should be fixed in the plans before execution begins. The remaining LOW concerns can be addressed during implementation without significant rework.

---

## OpenCode Review (Cycle 4 -- Revised Plans, Post-Cycle-3 Fixes)

# Cross-AI Plan Review: Phase 61 — Approval Workflow Engine (Revised)

## Summary

The revised plans thoroughly address all prior HIGH concerns and most MEDIUM/LOW items. The architecture is sound — JWT-based tokens with Ed25519, encrypted key storage via Phase 47 Envelope, TOCTOU guards on status transitions, LabelService-based cache key resolution, and T4 canonical message format with jti anti-replay. The four-wave dependency ordering (Foundation → Server API → Agent → TUI) is logical. However, three new MEDIUM-severity gaps emerged: (1) the revocation push mechanism is hand-waved as "cache delta" with no protocol detail, (2) the agent-side "use" audit event is absent from Plan 03, and (3) token delivery has no retry mechanism when the agent is unreachable. The plans are *almost* ready — these three items need explicit task additions.

---

## Strengths

- **TOCTOU guard correctly implemented**: `WHERE status='pending'` + `rows_affected` check + 409 on conflict is the right pattern for race-safe state transitions
- **T4 canonical message format** includes `jti` (approval ID) in the signed payload — deterministic string avoids JSON field-ordering ambiguity and provides anti-replay via server-side jti dedup
- **JWT re-verification on every cache read** is the correct security posture despite small performance cost; Ed25519 verify takes ~50µs
- **Cache key uses data_object_id** (not raw path) resolved through LabelService — correctly handles path aliases and renames
- **Esc from detail preserves list context** — avoids the common TUI sin of resetting state on navigation
- **Expiry picker with presets** (1h/4h/8h/24h/custom) is well-designed for enterprise UX
- **Board public key update gated to dlp-admin** with audit event — proper ACL
- **Encrypted signing key** via Phase 47 Envelope instead of env var is the correct approach
...
None. All prior HIGH concerns are demonstrably resolved in the plan text.

### MEDIUM

**M1 — Revocation push mechanism is underspecified (MEDIUM)**
Plan 04 says "pushes cache delta to agent" but no endpoint or protocol is defined for server→agent revocation push. Plan 03 has `Pipe1AgentMsg::ApprovalRejected` (for request rejection), but there is no `ApprovalRevoked` variant for invalidating an already-granted token. Needs: a `POST /agent/approval-revoke` endpoint or a generic cache-invalidation message, plus the agent handler that removes the DashMap entry.

**M2 — Agent-side "use" audit event missing (MEDIUM)**
WORKFLOW-06 requires "every approval request, grant, **and use**" to emit an audit event. Plans cover request (server) and grant/reject/revoke (server), but Plan 03 — where the agent consumes the token on a cache hit — does not document an audit event emission. The agent must emit an `ApprovalUse` audit event whenever a cached approval causes `evaluate()` to return ALLOW. Also requires new `AuditEvent` variant in dlp-common.

**M3 — Token delivery has no retry mechanism (MEDIUM)**
Plan 02 says "Server pushes token to agent via HTTP POST" — but a transient network blip or agent restart between grant and push means the token is lost. The server marks the approval as "approved" and the token is signed, but the agent never receives it. The user sees "approved" in the TUI but the agent still denies the operation. Needs: either (a) agent polls for pending tokens on startup/reconnect, or (b) server maintains a delivery queue with exponential backoff, or (c) the agent fetches approval state lazily on cache miss (which would naturally recover).

### LOW

**L1 — Server public key distribution to agent unspecified (LOW)**
Plan 03 re-verifies JWT signatures on every cache read, but where does the agent get the server's Ed25519 public key? If this is the *same* key used for admin auth JWTs, the agent already has it. If a *separate* key, a distribution mechanism (e.g., embedded in the token delivery response, or a `GET /server/public-key` endpoint) must exist. Should be documented to avoid integration surprises.

**L2 — ApprovalRejected vs ApprovalRevoked naming gap (LOW)**
Plan 03 introduces `Pipe1AgentMsg::ApprovalRejected` — the name implies the request was denied (never granted). For revocations of already-granted approvals, the agent needs a separate signal. Either rename to `ApprovalDenied` and add `ApprovalRevoked`, or clarify that `ApprovalRejected` serves both purposes.

**L3 — LabelService resolution failure unspecified (LOW)**
Plan 03 resolves `resource.path → data_object_id` via LabelService for cache key derivation. Behavior when LabelService is unavailable or path is unrecognized is not stated. Should be: "treat as no-match (cache miss), fall through to standard ABAC evaluation" rather than blocking.

**L4 — Phase 47 Envelope API compatibility (LOW)**
Plan 01 assumes Phase 47's `Envelope` abstraction exists and has `encrypt(key_bytes) → Vec<u8>` / `decrypt(ciphertext) → Vec<u8>`. If Phase 47 hasn't stabilized its API, this blocks Wave 1. Recommend: Task 0 spike should also verify the `Envelope` interface is available and compatible with ed25519-dalek key material size.

---

## Suggestions

1. **M1 fix**: Add to Plan 03 Task 2: "Add `POST /agent/approval-revoke` endpoint that removes cached entry by (sub, obj, act); add `Pipe1AgentMsg::ApprovalRevoked` variant to IPC enum"

2. **M2 fix**: Add to Plan 03 Task 1: "Emit `AuditEvent::ApprovalUse` when cached token allows an operation — include `requester_sid`, `data_object_id`, `action`, `token_id` (jti), `device_fingerprint`"

3. **M3 fix**: Add to Plan 02 Task 2: "Implement token delivery retry — queue failed deliveries, retry 3× with 1s/5s/30s backoff; agent fetches pending approvals from server on startup via `GET /agent/pending-approvals?since=<timestamp>`"

4. **L1 fix**: Add one line to Plan 03 Task 1: "Agent obtains server Ed25519 public key at startup via `GET /api/v1/server/public-key`; cache for 24h"

5. **L2 fix**: Rename `ApprovalRejected` → `ApprovalDenied` in Plan 03 Task 2, and add `ApprovalRevoked` for post-grant invalidation

6. **L3 fix**: Add to Plan 03 Task 1: "If LabelService resolution fails or path is unrecognized, treat as cache-miss — do NOT block the operation"

7. **L4 fix**: Add to Plan 01 Task 0: "Verify Phase 47 Envelope API accepts and returns Vec<u8> of appropriate size for ed25519-dalek PrivateKey bytes (32 bytes secret seed)"
...
No HIGH concerns remain. The three MEDIUM items — revocation push protocol, agent-side audit event, and delivery retry — are scoped, well-understood gaps that are easy to fix with explicit task additions. None are architectural errors; all are implementation details that were skimmed in the revision pass. With the suggested additions, the plans would be **LOW risk** and ready for execution.

The dependency on Phase 47 (Envelope) and Phase 60 (Data Owner routing) is the structural risk — if either phase slips, Phase 61 grinds to a halt. This is inherent to sequential-phase dependencies and acceptable given the roadmap ordering.
...
## Consensus Summary (Across All Cycles Including Cycle 4)

### Cycle 1 -> Cycle 2 -> Cycle 3 -> Cycle 4: Resolution Status
...
All Cycle 2/3 NEW concerns have been addressed in plan revisions.
Cycle 3 NEW concerns have been addressed in plan revisions.

### Cycle 4: New / Remaining Concerns

| Concern | Severity | Reviewer | Status |
|---------|----------|----------|--------|
| Cross-crate type dependency (CachedApproval references ApprovalClaims in dlp-server) | **HIGH** | Claude (Cycle 4) | **NEW** |
| Agent cannot verify approval tokens offline (no public key distribution) | **HIGH** | Claude (Cycle 4) | **NEW** |
| Revoke TOCTOU guard functionally broken (update_state hardcodes WHERE status=pending) | MEDIUM | Claude (Cycle 4) | **NEW** |
| /agent/approval-token endpoint location contradicts push architecture | MEDIUM | Claude (Cycle 4) | **NEW** |
| Approval audit event types not wired into SIEM routing | MEDIUM | Claude (Cycle 4) | **NEW** |
| Migration mechanism may conflict with existing conventions | MEDIUM | Claude (Cycle 4) | **NEW** |
| PolicyStore type referenced from agent code | LOW | Claude (Cycle 4) | **NEW** |
| T4 board signature out-of-band channel unspecified | LOW | Claude (Cycle 4) | **NEW** |
| chrono::Duration::hours may not exist | LOW | Claude (Cycle 4) | **NEW** |
| Revocation push mechanism underspecified | MEDIUM | OpenCode (Cycle 4) | **NEW** |
| Agent-side use audit event missing | MEDIUM | OpenCode (Cycle 4) | **NEW** |
| Token delivery has no retry mechanism | MEDIUM | OpenCode (Cycle 4) | **NEW** |
| Server public key distribution to agent unspecified | LOW | OpenCode (Cycle 4) | **NEW** |
| ApprovalRejected vs ApprovalRevoked naming gap | LOW | OpenCode (Cycle 4) | **NEW** |
| LabelService resolution failure unspecified | LOW | OpenCode (Cycle 4) | **NEW** |
| Phase 47 Envelope API compatibility | LOW | OpenCode (Cycle 4) | **NEW** |

### Recommended Actions Before Execution

1. **HIGH -- Fix cross-crate type dependency**: Move ApprovalClaims to dlp-common/src/approval.rs.
2. **HIGH -- Add public key distribution**: Add GET /agent/approvals/public-key endpoint and agent-side key caching.
3. **MEDIUM -- Fix revoke TOCTOU**: Change update_state to accept expected_current_status parameter.
4. **MEDIUM -- Clarify token delivery architecture**: Document whether push or poll.
5. **MEDIUM -- Wire approval events to SIEM**: Explicitly document SIEM routing.
6. **MEDIUM -- Add revocation push protocol**: Add POST /agent/approval-revoke endpoint.
7. **MEDIUM -- Add agent-side use audit event**: Emit ApprovalUse audit event on cache hit.
8. **MEDIUM -- Add token delivery retry**: Queue failed deliveries with exponential backoff.
9. **LOW -- Define ApprovalCache in dlp-agent**: Create agent-local approval cache type.
10. **LOW -- Document T4 OOB channel**: Add operational note about board member workflow.
11. **LOW -- Verify chrono API**: Use chrono::Duration::try_hours() or chrono::TimeDelta::try_hours().
12. **LOW -- Verify Phase 47 Envelope API**: Confirm Envelope interface compatibility.

---

*Review generated by cross-AI peer review (Claude CLI + OpenCode).*
*Cycle 1: 2026-05-13T01:05:00Z. Cycle 2: 2026-05-13. Cycle 3: 2026-05-13T10:30:00Z. Cycle 4: 2026-05-13T07:28:57Z.*
*To incorporate feedback: /gsd-plan-phase 61 --reviews*
