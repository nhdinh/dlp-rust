...
expected: cargo test -p dlp-hook-dll passes with 253 tests passing, 0 failed, 1 ignored.
result: pass
...
- The opt-in ignored chaos test `ntdll_chaos_test` crashes with STATUS_ACCESS_VIOLATION when run with `--ignored`. This is tracked as a follow-up bug in beads issue `dlp-rust-aao`. It is not part of the normal test suite and does not block Phase 51 verification.
