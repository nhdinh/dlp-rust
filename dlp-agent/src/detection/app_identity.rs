...
//!
//! Ported from `dlp-user-ui/src/detection/app_identity.rs` (Phase 25) to
//! `dlp-agent` (Phase 40) so drag-and-drop and file interception can resolve
//! source application identity in the agent process.
//!
...
fn run_wintrust(image_path: &str) -> i32 {
    ...
    let hr = unsafe {
        WinVerifyTrust(
            HWND::default(), // hwnd = NULL (no UI owner)
            &mut policy_guid,
...
fn resolve_uwp_identity(process_handle: HANDLE, image_path: &str) -> Option<(String, String)> {
    ...
    unsafe {
        let mut len: u32 = 0;
        let hr = GetApplicationUserModelId(process_handle, &mut len, None);
