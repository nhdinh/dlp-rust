...
//!
//! Connects to an AD LDAP server to resolve two attributes needed by
//! the ABAC engine:
...
//! ## Caching
//!
//! All lookups are cached with a configurable TTL (default 5 minutes)
//! using an in-memory `DashMap`. Cache hits avoid all network I/O.
...
use crate::policy_engine_error::{AdClientError, PolicyEngineError, Result};
...
/// An Active Directory client backed by LDAP with an in-memory TTL
/// cache.
///
/// All network I/O is performed on a blocking thread via
/// `tokio::task::spawn_blocking` so the async runtime is never blocked.
#[derive(Clone)]
pub struct AdClient {
    /// LDAP URL (e.g. "ldaps://dc01.contoso.com:636").
    ldap_url: String,
    ...
    network_location_cache:
        DashMap<String, CacheEntry<NetworkLocation>>,
}
...
impl AdClient {
    ...
    /// # Arguments
    ///
    /// * `ldap_url` - LDAP URL (e.g. "ldaps://dc01.contoso.com:636")
    /// * `base_dn` - Base DN (e.g. "DC=contoso,DC=com")
    ...
    pub async fn get_group_sids(
        ...
        let results =
            self.ldap_search(&filter, &["memberOf"]).await?;
    ...
    /// Looks up the device trust level for the machine identified by
    ...
    pub async fn get_device_trust(
        ...
        if let Some(entry) =
            self.device_trust_cache.get(machine_dn)
        {
            if !entry.is_expired() {
        ...
        let results =
            self.ldap_search(&filter, &[ATTR_DEVICE_TRUST]).await?;

        let trust = {
            let mut trust_str = String::new();
            for entry in results {
                if let Some(v) =
                    entry.attrs.get(ATTR_DEVICE_TRUST)
                {
    ...
    pub async fn get_network_location(
        ...
        if let Some(entry) =
            self.network_location_cache.get(machine_dn)
        {
            if !entry.is_expired() {
                debug!(
                    machine_dn,
                    "network location cache hit"
                );
                return Ok(entry.value.clone());
            }
        }
        ...
        let results = self
            .ldap_search(&filter, &[ATTR_NETWORK_LOCATION])
            .await?;
        ...
        let location = {
            ...
            for entry in results {
                if let Some(v) =
                    entry.attrs.get(ATTR_NETWORK_LOCATION)
                {
                    if let Some(s) = v.first() {
    ...
    ///
    /// The blocking LDAP operation is run on a dedicated thread to
        ...
        let entries: Vec<SearchEntry> =
            tokio::task::spawn_blocking(move || {
                let mut ldap = LdapConn::new(&url)
                    .map_err(|e| {
                        AdClientError::LdapInitError(e.to_string())
                    })?;

                ldap.simple_bind(&bind_dn, &bind_password)
                    .map_err(|e| {
                        AdClientError::BindFailed(e.to_string())
                    })?
                    .success()
                    .map_err(|e| {
                        AdClientError::BindFailed(e.to_string())
                ...
                let (entries, _result) = ldap
                    ...
                    .map_err(|e| {
                        AdClientError::AdQueryError(e.to_string())
                    })?
                    .success()
                    .map_err(|e| {
                        AdClientError::AdQueryError(e.to_string())
            ...
            .await
            .map_err(|e| {
                AdClientError::TaskJoinError(e.to_string())
            })?
...
/// Escapes special characters in LDAP filter values to prevent
/// injection.
...
/// Parses a `dlpDeviceTrust` attribute value into a `DeviceTrust`
/// enum.
...
/// Parses a `dlpNetworkLocation` attribute value into a
/// `NetworkLocation` enum.
...
///
/// Mock AD DN format:
/// `CN=GroupName,SID=<sid>,CN=Users,DC=mock,DC=local`
...
mod tests {
    ...
    fn test_extract_sid_from_dn() {
        assert_eq!(
            extract_sid_from_dn(
                "CN=Admins,SID=S-1-5-21-123,CN=Users,\
                 DC=mock,DC=local"
            ),
        ...
        assert_eq!(
            extract_sid_from_dn(
                "CN=Group,CN=Users,DC=mock,DC=local"
            ),
            None
        );
    }
    ...
    fn test_parse_device_trust() {
        assert_eq!(
            parse_device_trust("managed").unwrap(),
            DeviceTrust::Managed
        );
        assert_eq!(
            parse_device_trust("MANAGED").unwrap(),
        ...
        assert_eq!(
            parse_device_trust("").unwrap(),
            DeviceTrust::Unknown
        );
        assert_eq!(
            parse_device_trust("unknown").unwrap(),
            DeviceTrust::Unknown
        );
    }
