//! Integration tests for the DLP Server policy evaluation and CRUD API.
//!
//! Each test spawns a standalone server on an ephemeral port with a
//! fresh temporary policy store, exercises the endpoints via `reqwest`,
...
///
/// Returns `(base_url, tempdir, server_handle)`. The tempdir must be
/// kept alive for the duration of the test to prevent the policy file
/// from being deleted.
async fn spawn_server() -> (
    ...
    let store =
        Arc::new(PolicyStore::open(policy_path, engine).unwrap());
    let app = policy_api::router(store)
...
fn t3_unmanaged_deny_policy() -> Policy {
    Policy {
        ...
        description: Some(
            "Block T3 from unmanaged devices".into(),
        ),
        priority: 2,
        conditions: vec![
...
async fn test_evaluate_deny_t4() {
    ...
    assert_eq!(
        body.matched_policy_id.as_deref(),
        Some("pol-t4-deny")
    );
}
...
async fn test_evaluate_t3_unmanaged_denied() {
    ...
    let mut req =
        make_eval_request(Classification::T3, Action::COPY);
...
async fn test_evaluate_all_three_abac_rules() {
    ...
    let mut req_t3 =
        make_eval_request(Classification::T3, Action::WRITE);
